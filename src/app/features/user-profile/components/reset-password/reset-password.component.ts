...
export class ResetPasswordComponent implements OnInit, OnDestroy {
    ...
    this.resetPasswordForm = this.formBuilder.group(
      {
        ...
        newConfirmPassword: ['', [Validators.required, Validators.minLength(6), Validators.maxLength(15)]],
      },
      {
        validator: this.ConfirmPasswordValidator('newPassword', 'newConfirmPassword'),
      },
    );
  }
  ...
  reAuthenticateUser(): void {
    ...
    this.resetPasswordService
      .reauthenticateUser(this.currentUser, this.currentUser.email, formValue.oldPassword)
      .then((res) => {
        if (!res.user) {
  ...
  resetPassword(newPassword): void {
    this.resetPasswordService.resetPassword(this.currentUser, newPassword).then((res) => {
      this.manualSpinnyService.spin$.next(false);
      if (!res) {
